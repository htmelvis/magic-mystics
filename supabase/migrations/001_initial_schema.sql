...
CREATE TABLE public.ppf_readings (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
...
CREATE INDEX idx_reflections_reading_id ON public.reflections(reading_id);
CREATE INDEX idx_reflections_user_id ON public.reflections(user_id);
CREATE INDEX idx_ppf_readings_user_id ON public.ppf_readings(user_id);
CREATE INDEX idx_ppf_readings_created_at ON public.ppf_readings(created_at DESC);
